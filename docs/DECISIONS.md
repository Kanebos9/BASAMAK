...
| 65 | ABSOLUTE sound-browse knob (ui_sound_knobA) = delta-sign per message + RATCHET ends: the 16 values nearest each end are a free-rewind zone (motion away from that end = no steps) | Launchkey MK4 custom modes have NO relative option AND send nothing once pegged (user's MIDI monitor stayed grey) - the ratchet makes an absolute knob endless (crank / flick back free / crank); cost = a few swallowed clicks when deliberately reversing from very near an end, disclosed | my call, sourced + disclosed |
