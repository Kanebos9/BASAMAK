...
| 65 | ABSOLUTE sound-browse knob (ui_sound_knobA) = delta-sign per message + STATEFUL ratchet pawl: touching an end makes the WHOLE rewind free (any size); pawl releases on cranking again or a ~4 s pause | Launchkey MK4 custom modes have NO relative option AND send nothing once pegged; a stateless 16-value zone failed (flick acceleration outran it = backward steps mid-flick, user furious); deliberate reverse after a peg = pause a beat first (tooltip) | my call, sourced + disclosed |
| 66 | 14-bit knob path (ui_sound_knob14) = the FINE partner CC (+32) with wrap-aware delta (|diff|>64 = continued motion), no pawl; switch = a menu item after a normal learn; NEXT/PREV gained hold-to-repeat (450 ms arm, 220 ms rate, 15 s toggle-pad safety cap) | user rejected all rewind gestures; 14-bit puts the wall ~16k clicks away = endless in practice; the +32 convention is standard MIDI (coarse/fine pairs); hold-to-repeat = the zero-drama fallback | user-driven; +32 assumption disclosed |
