...
| 72 | The 3 new FX (user asked "what would shape the sound good?") = Crush (bitcrush+downsample) / Air (hi-shelf presence) / Ring (200 Hz ring mod) | fills the 3 obvious gaps left after Tone/Punch/Comp (grit, top-end sheen, inharmonic colour); each is one-knob, per-voice, matrix-targetable, and 0 = bypass = bit-identical | my pick inside user order |
