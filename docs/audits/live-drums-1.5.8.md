...
`PresetTest` checks every kit row for finite, audible output, an authored sound, and unique MIDI routing. The first eight assignments follow the physical OKTO B pad layout: Crash/Tom 1/Tom 2/Ride, then Kick/Snare/Tom 3/Hi-hat. `LiveDrumTest` also sends those eight notes through the actual processor while another channel is selected, checking the recorded channel order.
