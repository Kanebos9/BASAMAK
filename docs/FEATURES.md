...
| Part generator (v1.5.4) | GENERATE in the roll: role-based (bassline/melody/hum/riff), LINES structure (how many lyric-like sentences + repeat/answer/new relations; AABA emerges from Auto; cadences guaranteed), hit-exact rhythm (locks onto 7/11-step grooves precisely), per-beat harmony, seed-locked iteration (same-rhythm/same-notes/vary), one-shot dice, singable mode | Captain Melody / Scaler 2 phrases / Orb Producer | [+] rule-based like theirs, but ours reads the user's OWN groove (incl. odd step counts - mainstream tools can't), speaks in lyric lines instead of theory jargon, writes editable roll notes; vocal-guide (hum + record-your-own-hum multisample) combo is unique |
