                                               ...
                                               juce::MidiBuffer& midi, juce::AudioPlayHead* sliceHead)
{
        ...
        if ((sequencer.drums.enabled || (sequencer.drums.learnChannel >= 0 && msg.isNoteOn())) && msg.isNoteOnOrOff()) {
            if (msg.isNoteOn())
                sequencer.drums.noteOn(msg.getNoteNumber(),msg.getChannel(),msg.getVelocity(),
                    juce::jlimit(0, audio.getNumSamples()-1,meta.samplePosition)*kEngineOS);
            continue; // drum note-offs never truncate natural decay or reach the keyboard
        }
        // MIDI learn
    ...
    if (!sequencer.drums.enabled)
    {
        ...
        // MERGE & SPLIT pair (channel-wide): resolved once per block, used by routing/recording/suppression.
        const int mergedP = juce::jlimit(-1, Sequencer::NUM_CHANNELS - 1,
                                         sequencer.patterns[keyPat].channels[chIdx].mergeWith);
        const bool paired  = mergedP >= 0 && mergedP != chIdx;
        const int firstCh  = paired ? juce::jmin(chIdx, mergedP) : chIdx;   // RIGHT half of the keyboard
        const int secondCh = paired ? juce::jmax(chIdx, mergedP) : chIdx;   // LEFT half
        ...
        {
            if (paired)   // a merged pair is PIANO-ROLL only; make sure both are while recording
            {
                auto& pp = sequencer.patterns[keyPat].channels[mergedP];
                if (! pp.drawMode) { pp.drawMode = true; pp.clearDrawNotes(); }
            }
        ...
        sequencer.recordSuppressCh2.store((rec && drawRec && paired) ? mergedP : -1, std::memory_order_relaxed);
        ...
        if (rec && sequencer.isCurrentlyPlaying)
        {
            ...
            else if (lc != seenL || pp2 != seenP)
            {
                ...
                if ((chain || pp2 == armed || seenP == armed) && ! paired)   // a boundary we record across (merged PAIRS record looper-style, like groups: no per-loop takes/clears)
                {
                    if (drawRec)
                    {
        ...
        // MERGE & SPLIT routing: with the selected channel paired, keys at/above middle C play the
        // pair's FIRST (lower-numbered) channel, keys below it the SECOND - each half mapped onto its
        // 4-octave window (stored on the FIRST channel). Unpaired = identity onto the selected channel.
        auto splitMap = [&](int n, int& tgtCh) {
            if (! paired) { tgtCh = chIdx; return n; }
            const auto& fc = sequencer.patterns[keyPat].channels[firstCh];
            if (n >= 60) { tgtCh = firstCh;  return juce::jlimit(0, 127, fc.keysSplitW1 + (n - 60)); }
        ...
        auto handleKeyDown = [&](int note, float vel)
        {
            ...
            if (keysHeldCount < (int) (sizeof(keysHeldStack) / sizeof(keysHeldStack[0])))
            {
                ...
                int openIdx = -1, openPat = -1, openChan = tgtCh;
                if (rec && drawRec && (sequencer.isCurrentlyPlaying || kicked)
                    && std::abs(playNote - 60) <= DrumChannel::PITCH_RANGE)   // beyond the roll's grid: play live, don't record
                {
                ...
                keysHeldOpenChan[keysHeldCount] = openChan;
                ++keysHeldCount;
            }
        ...
        auto handleKeyUp = [&](int note)
        {
            ...
            {
                auto& pch = sequencer.patterns[juce::jlimit(0, Sequencer::NUM_PATTERNS - 1, openPat)]
                                .channels[juce::jlimit(0, Sequencer::NUM_CHANNELS - 1, openChan)];
        ...
        if (rec && ! drawRec && held >= 0 && sequencer.isCurrentlyPlaying)
        {
            ...
            if (chain || pat == keysArmedPattern.load(std::memory_order_relaxed))
            {
                ...
                const int last = keysLastStampStep.load(std::memory_order_relaxed);
                int hTgt = chIdx; const int heldMapped = paired ? held : splitMap(held, hTgt);
                if (! paired && last >= 0 && cur != last && cur == (last + 1) % n && cur != 0)   // no wrap-merge across the loop (pairs are roll-only)
                {
                    logEvt(pat, cur, heldMapped - 60, 1);               // flags bit0 = merge
    ...
    if (sequencer.drums.enabled) {
        sequencer.recordSuppressCh.store(-1); sequencer.recordSuppressCh2.store(-1);
        sequencer.recordLoopLock.store(sequencer.drums.recording && !sequencer.drums.followChain);
        if (sequencer.drums.recording && sequencer.drums.inputCount > 0 && !sequencer.isCurrentlyPlaying && !sequencer.dawSync)
            sequencer.startStandalone();
        // Discard stale on-screen keyboard messages while its controls are disabled.
    ...
    //   flushes everything so nothing hangs in the target plugin.
    {
        ...
        for (auto& e : events)
        {
            const int ep = e.pattern >= 0 ? e.pattern : sequencer.playPattern;
            auto& ch = sequencer.patterns[ep].channels[e.channel];
            if (ch.mute || (Sequencer::anySoloIn(sequencer.patterns[ep]) && !ch.solo)) continue;
            ...
            const int note  = e.drumHit ? juce::jlimit(0,127,ch.midiNote) : e.isDraw ? juce::jlimit(0, 127, 60 + juce::roundToInt(e.drawPitch))
            ...
            int len;
            if (e.drumHit) len = juce::jmax(1,(int)(currentSampleRate * 0.01));
            else if (e.isDraw) len = juce::jmax(1, (int) (e.gate / kEngineOS));
            else {
...
void DrumSequencerProcessor::routeCC(const juce::MidiMessage& msg)
{
    ...
    if (sequencer.drums.enabled && (pid.startsWith("ui_mode_") || pid.startsWith("ui_selstep_")
        || pid.startsWith("ui_patstep_") || pid.contains("_step"))) return;
    ...
    if (pid == "global_stop")       { if (on && !sequencer.dawSync) standaloneStop(); return; }
...
juce::File DrumSequencerProcessor::exportMidiFile(int channel)
{
    ...
    if(sequencer.drums.enabled) {
        const juce::ScopedLock lock(getCallbackLock());const auto& d=sequencer.drums;
        int head=sequencer.groupHead(sequencer.currentPattern),end=sequencer.groupEnd(head);
        for(int p=head;p<=end;++p)for(int i=0;i<d.patterns[(size_t)p].count;++i){const auto& h=d.patterns[(size_t)p].hits[(size_t)i];
    ...
    for (int gBar = gHead; gBar <= gEnd; ++gBar)
    {
        ...
        if (chn.drawMode)
        {
            ...
            for (int ni = 0; ni < chn.drawNoteCount; ++ni)   // note list: recorded/drawn chords export as chords
            {
                ...
                if(nt.drumHit) {
                    const int note=chn.midiOut?chn.midiNote:(sequencer.drums.notes[(size_t)ch]>=0?sequencer.drums.notes[(size_t)ch]:chn.midiNote);
                    seq.addEvent(juce::MidiMessage::noteOn(midiCh,note,vel),tickOff+nt.start*colTicks);
                    seq.addEvent(juce::MidiMessage::noteOff(midiCh,note),tickOff+(nt.start+1)*colTicks);
                    continue;
                }
...
static void writeChannel(juce::ValueTree& chState, const Channel& ch)
{
    ...
    chState.setProperty("mergeWith",  ch.mergeWith,    nullptr);   // MERGE&SPLIT: paired channel (-1 = none)
...
static void readChannel(const juce::ValueTree& child, DrumChannel& ch)
{
    ...
    ch.mergeWith   = juce::jlimit(-1, Sequencer::NUM_CHANNELS - 1, (int) child.getProperty("mergeWith", -1));
...
void DrumSequencerProcessor::copyPattern(int src, int dst)
{
    ...
    if(sequencer.drums.enabled)sequencer.drums.patterns[(size_t)dst]=sequencer.drums.patterns[(size_t)src];
...
void DrumSequencerProcessor::copyChannel(int pat, int src, int dst)
{
    ...
    if(sequencer.drums.enabled && src!=dst && pat>=0&&pat<64&&src>=0&&src<16&&dst>=0&&dst<16) {
        ...
        for(int i=0;i<lane.count;++i){nSrc+=lane.hits[(size_t)i].channel==src;nDst+=lane.hits[(size_t)i].channel==dst;}
        if(lane.count-nDst+nSrc>LiveDrumming::MAX_HITS)return;
        for(int i=lane.count-1;i>=0;--i)if(lane.hits[(size_t)i].channel==dst)lane.erase(i);
        const int count=lane.count;for(int i=0;i<count;++i)if(lane.hits[(size_t)i].channel==src){auto hit=lane.hits[(size_t)i];hit.channel=dst;lane.add(hit);}
    }
